...
open3d()
for (i in 1:length(PartLevels)){
  spheres3d(Wom_cranium_Ref[Part[[i]],1], Wom_cranium_Ref[Part[[i]],2], Wom_cranium_Ref[Part[[i]],3], col=Colours[i], lit=TRUE,radius = 0.4, asp=F)
  
}

#Add text to see which LM number is a curve/fixed LM/ patch; Viewbox is really chaotic in the order of curve points!
text3d(Wom_cranium_Ref, text=c(1:826), adj=1.2, cex=0.8)
