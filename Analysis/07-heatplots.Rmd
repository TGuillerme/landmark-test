...
#Trying to do this using hypothetical PC min/max landmarks. T
...
heatplot.PCs<-function (species_dataset,minfirst, PC_axis,...){
  ...
  if(minfirst==TRUE){
    open3d()
    plotRefToTarget.heat(PCA$pc.shapes$PC2min,
                         PCA$pc.shapes$PC2max,
                         col = heat.colors, pt.size = 0.7, col.val = procrustes_var,
                         plotRefToTarget.args = list(mag = 1),...)}
  else {
    open3d()
    plotRefToTarget.heat(PCA$pc.shapes$PC2max,
                         PCA$pc.shapes$PC2min,
                         col = heat.colors, pt.size = 0.7, col.val = procrustes_var,
                         plotRefToTarget.args = list(mag = 1), ...)}
}


heatplot.PCs(AllWombats$cranium, minfirst=FALSE, )
heatplot.PCs(HN$cranium, minfirst=TRUE  )
heatplot.PCs(CW$cranium, minfirst=TRUE)
heatplot.PCs(SHNW$cranium, minfirst=TRUE)
heatplot.PCs(NHNW$cranium, minfirst=TRUE)


heatplot.PCs(AllWombats$mandible, minfirst=FALSE, )
heatplot.PCs(HN$mandible, minfirst=TRUE )
heatplot.PCs(CW$mandible, minfirst=FALSE)
heatplot.PCs(SHNW$mandible, minfirst=TRUE)
heatplot.PCs(NHNW$mandible, minfirst=TRUE)
