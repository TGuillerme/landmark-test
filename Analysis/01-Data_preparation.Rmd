...
for(chain in chains) {
    ...
    for(dataset in datasets) {
        ...
        ## Read the landmarks
        landmarks  <- t(read.csv(paste0("../Data/landmarkdata_", dataset, "_", chain, ".csv"),
                                 header = TRUE, row.names = 1))
        
        ## Remove any NAs rows
        landmarks <- landmarks[complete.cases(landmarks),]

        ## Make into a 3D array
        landmarks <- geomorph::arrayspecs(landmarks, k = 3, p = ncol(landmarks)/3)

        ## Running the Procrustes superimposition
        land_data[[dataset]][[2]] <- geomorph::gpagen(landmarks, print.progress = FALSE)
...
for(sp in 1:length(species)) {
    ...
    for(dataset in datasets) {
        ...
        wombat_sp <- read.csv(paste0("../Data/wombat_species_", dataset, ".csv"))
        ...
        landmarks <- landmarks[, , which(wombat_sp$Species == species[sp])]
