...
#change "cranium" designations to "mandible" desiignations as needed
...
#create Surfslide file
  wombat_mandible_surfslide=which(landmark_LM_types$x==3)
  write.csv(wombat_mandible_surfslide, file="../Data/Processed/wombat_mandible_surfslide.csv")

#curveslide file created manually using the visualization code below and the Viewbox landmark names contained in the landmarkdata_mandible_wombat raw data file.

# Checking that the sliding semilandmarks were correctly chosen
wombat_mandible_curves=which(landmark_LM_types$x==2)
curveslide=read.csv(file="../Data/Processed/wombat_mandible_curveslide.csv")

#check that the sliding lms as designated in viewbox match with the lms designated in the curveslide file
setdiff(curveslide$slider, wombat_mandible_curves)
setdiff(wombat_mandible_curves, curveslide$slider)
...
Wom_mandible_Data <- t(rawdata_mandible_wombat) 
# Remove any empty rows
Wom_mandible_Data <- Wom_mandible_Data[complete.cases(Wom_mandible_Data),] 
# Make 3D Array
Wom_mandible_Data <- arrayspecs(Wom_mandible_Data, k=3, p=ncol(Wom_mandible_Data)/3) 

Wom_mandible_Ref <- Wom_mandible_Data[,,6]
