...
Where $n$ is the total number of landmarks and $f_{x}$ and $f_{y}$ are ranked functions (i.e. $f_{0} \geq f_{1} \geq f_{2} ...$) with respectively $n-m$ and $m$ total observations (with $m \leq n$).

### Probability of overlap

The Bhattacharyya Coefficient calculates the probability of overlap of two distributions. <!-- \citep{Bhattacharyya} -->
...
To test whether the landmark radii for one partition is different than the remaining ones, we can use a random test (modification from `ade4::as.randtest`).
