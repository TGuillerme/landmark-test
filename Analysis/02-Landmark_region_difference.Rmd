...
source("../Functions/bootstrap.test.R")
set.seed(1)
```
...
## Additional - Calculating the mean difference

```{r}
## Mean differences
means_diff <- lapply(partitions, lapply.bootstrap.test, data = procrustes_var, statistic = mean, repicates = 500, alternative = "greater")
```
The mean differences:

```{r, echo = FALSE}
## Size differences
kable(make.table(means_diff, "bonferroni"), digits = 5)
```

```{r, fig.width = 10, fig.height = 15, echo = FALSE}
## Plot the size differences
make.plots(means_diff, type = "mean difference", add.p = TRUE, correction = "bonferroni")
```
