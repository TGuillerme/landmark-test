...
names(Geo_procD_outputs)<-names(Geo_GDFs)
 ...
 Geo_procD_outputs<-list()
    ...
    for (k in 1:length(Geo_GDFs[[2]])){
      #the below code works by itself but won't loop; I am probably forgetting some return rule
  Geo_procD_outputs[[2]][[2]] <-procD.lm(coords~Temp, data=Geo_GDFs[[2]][[2]], iter=1000)
