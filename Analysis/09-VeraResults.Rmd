...
Cr_gdf<-geomorph.data.frame(coords=land_data$cranium$procrustes$coords, Csize=land_data$cranium$procrustes$Csize, species=as.factor(cranium$Species))
Mand_gdf<-geomorph.data.frame(coords=land_data$mandible$procrustes$coords, Csize=land_data$mandible$procrustes$Csize, species=as.factor(mandible$Species))
...
for (i in 1:length(Geo_GDFs)){
  ...
  Geo_procD_outputs[[i]][[k]] <-procD.lm(coords~Aridity, data=Geo_GDFs[[i]][[k]], iter=1000)
