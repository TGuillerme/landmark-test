...
for (i in 1:length(levels(cranium$Species)){
  PLS_PCA_Csize=list()
  for (k in 1:length(AllWombats)[[1]]){
    
    PLS_Scores=list("SHNW"=list (SHNWTBPLS$XScores, SHNWTBPLS$YScores),list (NHNWTBPLS$XScores, NHNWTBPLS$YScores), list (CWTBPLS$XScores, CWTBPLS$YScores))
    
    TBPLSScores[[i]][[k]] <-
    PC Scores [[i]][[k]]  <-
    Csize[[i]][[k]]       <-
      
    PLS_Scores <- as.data.frame(CWTBPLS$XScores], row.names=dimnames(CWTBPLS$XScores)[[1]])
...
#extract the data for merging
CWTBPLS_Cranium_Scores <- as.data.frame(CWTBPLS$XScores[c(1:18)], row.names=dimnames(CWTBPLS$XScores)[[1]])
CWPC1=as.data.frame(CW$cranium$ordination$x[,1])
CWCsize=as.data.frame(CW$cranium$procrustes$Csize)

#Merge, clean up merged data frame
CWPLS_PCA=merge(CWTBPLS_Cranium_Scores,CWPC1, by="row.names",all.x=TRUE)
row.names(CWPLS_PCA)=CWPLS_PCA$Row.names
CWPLS=merge(CWPLS_PCA,CWCsize,by="row.names",all.x=TRUE)
row.names(CWPLS)<-CWPLS[,1]
CWPLS=CWPLS[,3:5]
colnames(CWPLS)<-c("PLS_Score", "PC_Score", "Csize")
...
plot(CWPLS_PCA_separate[,2], CWPLS_PCA_separate[,3])
