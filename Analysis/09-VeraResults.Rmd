...
allom.shape<-function (procrustes_coordinate_file_with_centroid_size){
  ...
  Csize=procrustes_coordinate_file_with_centroid_size$Csize
    ...
    data_list <- list("coords"=coords, "Csize"=Csize)
  
   
  Allometry <- procD.allometry(coords~ Csize, data=data_list, f2=NULL, f3=NULL, logsz = FALSE, iter=999)
...
#TG: I've put the procD type functions in a "safe" wrapping function, you can now use them as follow (sourced from utilities.R)
handle.procD.formula(formula = coords ~ Csize, procrustes = land_data$cranium$procrustes, procD.fun = procD.allometry, logsz = FALSE, iter = 1)

#TG: The first term (coords ~ Csize) is the formula to test
#TG: the second (land_data$cranium$procrustes) is the procrustes data object,
#TG: the third is the function to apply to that (procD.allometry - it can be any procD.something function)
#TG: the following arguments (logsz and iter) are optional arguments to be passed to the procD function.


```
#2-Block Partial Least Squares analysis

##reduce classifier datasets to those that only have craniums and crania
```{r, message = FALSE, warning = FALSE}
...
for (i in 1:length(unique(cranium$Species))){
    ...
    sp_name=paste(species[i],deparse(substitute(cranium)),"TBPLS", "Class", sep="_")#prepare for giving names to outputs
...
for (i in 1:length(unique(mandible$Species))){
    ...
    sp_name=paste(species[i],deparse(substitute(mandible)),"TBPLS", "Class", sep="_")#prepare for giving names to outputs
...
##The below works, but not wrapped in a function. I am sure I am just missing something simple.The below can be deleted once the above is successfully wrapped
...
AllClassifiers=list("NHNW"=list("cranium"=NHNW_cranium_TBPLS_Class, "mandible"=NHNW_mandible_TBPLS_Class), "SHNW"=list("cranium"=SHNW_cranium_TBPLS_Class, "mandible"=SHNW_mandible_TBPLS_Class),"CW"=list("cranium"=CW_cranium_TBPLS_Class, "mandible"=CW_mandible_TBPLS_Class))
...
```{r, message = FALSE, warning = FALSE}
SHNWTBPLS <- two.b.pls(matched_coords[[1]]$SHNW$cranium ,matched_coords[[1]]$SHNW$mandible ,iter=1) 
summary(SHNWTBPLS)
...
NHNWTBPLS <- two.b.pls(matched_coords[[1]]$NHNW$cranium ,matched_coords[[1]]$NHNW$mandible, iter=1) 
 summary(NHNWTBPLS)
save(NHNWTBPLS, file="../Data/Results/NHNWTBLS.Rda")
load("../Data/Results/NHNWTBLS.Rda")
CWTBPLS <- two.b.pls(matched_coords[[1]]$CW$cranium ,matched_coords[[1]]$CW$mandible ,iter=1) 
 summary(CWTBPLS)
