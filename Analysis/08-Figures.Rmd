...
plot.partitions(land_data$mandible, PointSize = 0.01)
shade3d()
...
#text3d(mshape(land_data$mandible$procrustes$coords), text=c(1:577), adj = 1, cex = 0.5)
...
names <- dimnames(land_data$mandible$procrustes$coords)[[3]] # Gives all IDs in data
Species <- as.factor(mandible$Species)
Locality <- as.factor(mandible$Locality)
Environment <- as.factor(mandible$Environment) 
Sex <- as.factor(mandible$Sex)
...
plot.new()
ggplot(wombat_mandible_PC, aes(x=wombat_mandible_PC$wombat_mandible_PC1Scores, y=wombat_mandible_PC$wombat_mandible_PC2Scores, colour=Species, fill=Species))+
  geom_convexhull(alpha=.4)+
  scale_shape_manual(values = c(21, 22, 24))+
  
  geom_point(aes(shape=Sex, fill=Species), color="black", size=2, stroke = 0.5)+
  geom_text(aes(label=Environment), size=3, hjust=1.35, vjust=0.25, colour="black")+
...
ggsave("E:/Students/Cruise/Paper/Figures/wombat_mandible_PC.pdf",dpi = 600)
