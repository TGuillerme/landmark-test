...
```{r}
gdf=geomorph.data.frame(coords=land_data$cranium$procrustes$coords, Csize=land_data$cranium$procrustes$Csize)
AllomCranium=procD.allometry(coords~Csize, data=gdf, method="RegScore")


gdf=geomorph.data.frame(coords=land_data$mandible$procrustes$coords, Csize=land_data$mandible$procrustes$Csize)
...
par(mfrow = c(1,2)) 
plot(AllomCranium$Reg.proj~AllomCranium$size,bg=gg.color.hue(length(levels(cranium$Species)))[cranium$Species], pch=21, cex=1.5, xlab="Cranium Centroid size", ylab="Regression score")
plot(AllomMandible$Reg.proj~AllomMandible$size,bg=gg.color.hue(length(levels(mandible$Species)))[mandible$Species], pch=21, cex=1.5, xlab="Mandible Centroid size", ylab="Regression score")
