...
results_table <- data.frame(matrix(NA, ncol = length(species)*3, nrow = 4))
...
for(sp in 1:length(species)) {
    ...
    for(ds in 1:length(datasets)) {
        ...
        if(ds == 1) {
            ## Cranium
            results_table[1:2, 1+(3*(sp-1))] <- difference[,2]
            results_table[1:2, 2+(3*(sp-1))] <- sapply(difference[,6], get.token)
            results_table[1:2, 3+(3*(sp-1))] <- sapply(overlap[,6], get.token)
            ## Make the differences to be 0 if non-signif
        } else {
            ## Mandible
            results_table[3:4, 1+(3*(sp-1))] <- difference[,2]
            results_table[3:4, 2+(3*(sp-1))] <- sapply(difference[,6], get.token)
            results_table[3:4, 3+(3*(sp-1))] <- sapply(overlap[,6], get.token)
        }
    }
}
...
## Renaming the rows
rownames(results_table) <- c(paste0(datasets[1], 1:2), paste0(datasets[2], 1:2))
